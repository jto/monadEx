...
object Monad {
  def ListMonad: Monad[List] = new Monad[List]{
    def flatMap[A, B](a: List[A], f: A => List[B]) = (a map f) flatten
  ...
  def InterMonad: Monad[Inter] = new Monad[Inter]{
    def flatMap[A, B](a: Inter[A], f: A => Inter[B]): Inter[B] = Inter({x:Int =>
      (f compose a.f)(x).f(x)
    })
  ...
  def IdentityMonad: Monad[Identity] = new Monad[Identity]{
    def flatMap[A, B](a: Identity[A], f: A => Identity[B]) = f(a.a)
...
object MonadicFunctions {
  def sequence[M[_], A](as: List[M[A]], m: Monad[M]): M[List[A]] = {
    val reversed = as.foldLeft[M[List[A]]](m unital Nil){ (acc, ma) =>
      m.flatMap(ma, { a: A =>
        m.flatMap(acc, { as: List[A] =>
          m.unital(a :: as)
        })
      })
    }
    m.flatMap(reversed, { as:List[A] => m.unital(as.reverse) })
  }
